...
const List<String> customCropIcons = ['TeaLeafIcon', 'RiceGrainIcon', 'CropGenericIcon'];
...
String assetNameFor(String componentName) {
  ...
  return base.replaceAllMapped(RegExp('(?<=[a-z0-9])([A-Z])'), (m) => '_${m[1]}').toLowerCase();
}
...
String jsxToSvg(String jsx) {
  return jsx
      .replaceAllMapped(RegExp(r'([A-Za-z-]+)=(\{([^}]*)\}|"([^"]*)")'), (m) {
        ...
        final value = (m[3] ?? m[4]!).trim().replaceAll(RegExp(r'''^['"]|['"]$'''), '');
        ...
        if (m[3] != null && !literal.hasMatch(m[3]!.trim())) {
          throw FormatException('Cannot convert the JSX expression {${m[3]}} in $name');
        }
...
String iconSvg(String tsx, String file) {
  ...
  final svg = jsxToSvg(match[0]!).replaceFirst('<svg', '<svg xmlns="http://www.w3.org/2000/svg"');
  return '${_tidy(svg)}\n';
}

/// A default avatar: AvatarBadge's circle shell around the role's motif.
String avatarSvg(String tsx, String file) {
  final match = RegExp(r'<AvatarBadge[^>]*>([\s\S]*?)</AvatarBadge>').firstMatch(tsx);
  if (match == null) {
...
String catalogSource(List<CatalogEntry> entries) {
  final buffer = StringBuffer()
    ..writeln('// GENERATED by tool/import_web_icons.dart from the website\'s lib/cropCatalog.ts.')
...
void main(List<String> args) {
  ...
  final catalog = parseCatalog(File('$web/lib/cropCatalog.ts').readAsStringSync());
  for (final entry in catalog) {
    final asset = File('$cropAssetDir/${assetNameFor(entry.component)}.svg');
    if (!asset.existsSync()) {
      stderr.writeln('error: ${entry.value} uses ${entry.component}, which was not converted');
      exit(1);
    }
  }
