...
class Formatters {
  ...
  String number(num value) {
    final format = NumberFormat.decimalPattern(numberLocale)..maximumFractionDigits = 2;
    return format.format(value);
  }

  /// "Rs 1,250.5", formatted like the website's prices.
  String rupees(AppLocalizations l10n, num value) => l10n.commonUnitsRupees(number(value));

  String rupeesPerUnit(AppLocalizations l10n, num value) =>
      l10n.commonUnitsRupeesPerUnit(number(value));

  String kilograms(AppLocalizations l10n, num value) => l10n.commonUnitsKg(number(value));

  /// "25 Sept 2026" in the phone's time zone.
  String date(DateTime value) => DateFormat.yMMMd(dateLocale).format(value.toLocal());

  /// "25 Sept 2026, 3:40 PM" in the phone's time zone.
  String dateTime(DateTime value) => DateFormat.yMMMd(dateLocale).add_jm().format(value.toLocal());
}

final formattersProvider = Provider<Formatters>((ref) => Formatters(ref.watch(languageProvider)));
